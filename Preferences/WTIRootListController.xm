#import "WTIRootListController.h"
#import "spawn.h"

@implementation WTIRootListController
...
- (instancetype)init
{
    ...
    if (self)
	{
        WTIAppearanceSettings *appearanceSettings = [[WTIAppearanceSettings alloc] init];
        ...
        self.titleLabel.text = @"WeatherInfo";
...
{
    if (scrollView.contentOffset.y > [WTIRootHeaderView headerH] / 2.0) [UIView animateWithDuration: 0.2 animations: ^{ self.titleLabel.alpha = 1.0; }];
...
- (void)reset: (PSSpecifier*)specifier
{
    UIAlertController *reset = [UIAlertController
        alertControllerWithTitle: @"WeatherInfo"
	...
	UIAlertAction *confirmAction = [UIAlertAction actionWithTitle: @"Confirm" style: UIAlertActionStyleDestructive handler:
        ...
        {
            [[[HBPreferences alloc] initWithIdentifier: @"com.johnzaro.weatherinfoprefs"] removeAllObjects];

            NSFileManager *manager = [NSFileManager defaultManager];
            [manager removeItemAtPath:@"/var/mobile/Library/Preferences/com.johnzaro.weatherinfoprefs.plist" error: nil];
            [manager removeItemAtPath:@"/var/mobile/Library/Preferences/com.johnzaro.weatherinfoprefs.colors.plist" error: nil];
